...
st.set_page_config(page_title="HireHub - CV Parser App", layout="wide")
...
# Function to delete a specific CV
def delete_cv(cv_id):
    st.session_state.cvs = [cv for cv in st.session_state.cvs if cv['id'] != cv_id]
...
def main():
    st.markdown("<h1 style='text-align: center; font-family: Arial, Helvetica, sans-serif; color: #4CAF50;'>HireHub - CV Parser App</h1>", unsafe_allow_html=True)
    ...
    if choice == "Home":
        st.header("Welcome to HireHub")
        name = st.text_input("Enter your name")
        email = st.text_input("Enter your email")
        phone = st.text_input("Enter your phone number", value="", max_chars=15)
        ...
        if st.button("Send"):
            if email and phone and name:
                ...
                st.session_state.user_info.append(user_info)
                st.success("Information sent to the dashboard!")
                st.experimental_rerun()  # Refresh the home page
        ...
        for idx, cv in enumerate(filtered_cvs):
            with st.expander(f"CV ID: {cv['id']}   |   Name: {cv['name']}   |   Score: {cv['score']}   |   Date: {cv['upload_time']}"):
                ...
                if cv_file_path and isinstance(cv_file_path, str) and os.path.exists(cv_file_path):
                    ...
                    if st.button(f"View CV {cv['id']}", key=f"view_{cv['id']}"):
                ...
                # Add a button to delete the CV
                if st.button(f"Delete CV {cv['id']}", key=f"delete_{cv['id']}"):
                    delete_cv(cv['id'])
                    st.success(f"CV ID {cv['id']} deleted.")
                    st.experimental_rerun()
